...
!  integer, parameter :: max_lanc_vec_doublej=10
  integer, parameter :: max_lanc_vec_doublej=5
!  integer, parameter :: max_lanc_vec_doublej=3
  ...
  logical :: is_save_tmp = .true.
  integer :: ncnt_inner=0
  ...
  recursive subroutine lanczos_main(matvec, dotprod, ndim, eval, evec, &
    ...
    outer: do itr = 1, miter
       do iv = n_iv_start, max_lv-1
          call print_mem_status('l03-lan-iter')
          ...
          if (present(is_inner) .or. mod(iv-1, nskip_diag)==0 .or. iv==max_lv-1) then
             ...
             if (present(is_inner) .and. (ncnt_inner > 3) .and. &
                  abs(teval(1) - eval_jj) > 0.5d0) then 
                if (myrank==0) write(*,*) &
                     "possible JJ collapse : check bn converge condition, try add_randinit=0.1d0", &
                     teval(1), eval_jj
    ...
    call print_mem_status('l04-lan-fin-iter')

    if (myrank==0 .and. n_itr==miter) then
       if (present(is_inner)) then
          write(*,'(a)')"WARNING: JJ not converged, check <JJ> "
       else
          write(*,'(/,a)') "***********************************************************"
          write(*,'(3a)')  "*****    ", trim(c_inout)," NOT converged in Lanczos method  *****"
          write(*,'(a,i5,i5,a)') "*****  ", n_itr, max_lanc_vec,"                        *****"
          write(*,'(a,/)') "************************************************************"
       end if
    end if
...
!    call print_max_l_vec()
    ...
    call print_mem_status('l05-lan-fin')
...
!         write(*,*)" rename ",trim(fn_lv_new)//trim(c_num)," ", trim(fn_lv)//trim(c_num)
  ...
  subroutine set_lanczos_tmp_fn(fn_base, is_save)
    ...
    call set_dump_fname_mpi_sf(fn_base_dump)
