...
#is_mpi = True         # FX10 
#is_mpi = 'fugaku'     # Fugaku computer
#is_mpi = 'wisteria-o' # Wisteria
#is_mpi = 'pegasus'    # Pegasus at CCS Tsukuba

n_nodes = 16  # default number of MPI nodes 
#n_nodes = 768
if is_mpi in ('fugaku', ): n_nodes = 48
if is_mpi in ('pegasus',): n_nodes = 2
...
var_dict = {
    ...
    "eff_charge"    : [1.5, 0.5] , 
    "gl"            : [1.0, 0.0], 
    "gs"            : [ 5.585, -3.826], # [3.910, -2.678],  # [ 5.585, -3.826]
    "beta_cm"       : 0.0,
    "n_block"       : 0,  
    }
...
def exec_string(mode, fn_input, fn_log):
    ...
    if is_mpi in ('pegasus',): 
        return 'mpirun -hostfile ${PBS_NODEFILE}  -perhost 1  ' + fn_exe + fn_input + ' > ' + fn_log + '  \n\n'
#        return 'mpirun -hostfile ${PBS_NODEFILE} -perhost 1 --bind-to none ' + fn_exe + fn_input + ' > ' + fn_log + '  \n\n'
#        return 'mpirun -hostfile ${PBS_NODEFILE} -np '+ str(n_nodes) + ' -perhost 1 ' + fn_exe + fn_input + ' > ' + fn_log + '  \n\n'
...
#-----------------------------------------------------
    ...
    while True:
        ...
        list_param += [ 'is_obtd = .true.', 'is_tbtd = .true.', 
                        'is_calc_tbme = .true.', 'is_ry_sum = .true.',
                        'n_block = ', 'sq 0.7' ]
...
# ---------------------------------------------
    ...
    for mtot,nparity,n_eigen,is_proj in list_jpn:
        ...
        if trc_list_prty[nparity] is not None \
           and not ('orbs_ratio' in var_dict) : 
        ...
        var_dict[ 'max_lanc_vec' ] \
            = max( int(var_dict[ 'max_lanc_vec' ]), 
                var_dict[ 'n_restart_vec' ] + 50 )
        var_dict[ 'mtot' ] = mtot
        
        fn_input = fn_base + '_' + str(mtot) + '.input'

        #if var_dict.has_key('no_save'):
        if 'no_save' in var_dict:
            del var_dict[ 'fn_save_wave' ]

        out += 'echo "start running ' + fn_log + ' ..."\n' 
        out += 'cat > ' + fn_input + ' <<EOF\n' \
            +  '&input\n'
        out += print_var_dict( var_dict, skip=('is_obtd', 'is_tbtd', 'no_save') )
        out += '&end\n' \
            +  'EOF\n'
    ...
    for i1, (m1, np1, ne1, isj1) in enumerate(list_jpn):
        for i2, (m2, np2, ne2, isj2) in enumerate(list_jpn):
            if i1 > i2: continue
            # if (isj1 and m1==0) and (isj2 and m2==0): continue # E0
            is_skip = True
            if is_e2m1: 
                if abs(m1-m2) <= 4 and np1 == np2: is_skip = False
            if is_e1:
                if abs(m1-m2) <= 2 and np1 != np2: is_skip = False
                # if abs(m1-m2) <= 6 and np1 != np2: is_skip = False # for E3
            if is_skip: continue
...
def main():
    ...
    if cdef == True: cdef = 'Y'
    list_param = [ 'fugaku', 'wisteria-o', 'pegasus', 
    ...
    if is_mpi == 'fugaku':
        print( txt + 'on Fugaku supercomputer' )
    elif is_mpi == 'wisteria-o':
        print( txt + 'on Wisteria-O supercomputer' )
    elif is_mpi == 'pegasus':
        print( txt + 'on PEGASUS supercomputer' )
    elif is_mpi: 
    ...
    if ff_pair and ask_yn('First forbidden transition'):
        outsh += '# --- First forbidden transition beta-minus --- \n'
        outsh += 'echo \n'
        outsh += 'echo "First forbidden transition beta-minus calc."\n'
        outsh += output_transit_pair(ff_pair)
        #
        ff_p_pair = [ (j,i) for i,j in ff_pair ]
        outsh += '# --- First forbidden transition beta-plus --- \n'
        outsh += 'echo \n'
        outsh += 'echo "First forbidden transition beta-plus calc."\n'
        outsh += output_transit_pair(ff_p_pair)
    ...
    if tna_pair and ask_yn('Two-nucleon transfer amplitude'):
        outsh += '# --------- two-nucleon transfer amplitude --------- \n'
        outsh += 'echo \n'
        outsh += 'echo "two-nucleon transfer amplitude"\n'
        outsh += output_transit_pair(tna_pair)
